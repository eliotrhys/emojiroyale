  ...
  const allMediaTypes = Object.values(MediaType).map((mediaType, index) => 
    <div className="mb-6" key={index}>
      <input
        ...
        type="checkbox"
        role="switch"
        id={mediaType.replaceAll(/\s/g,'')} />
      <label htmlFor={mediaType.replaceAll(/\s/g,'')} className="inline-block pl-[0.15rem] text-sm hover:cursor-pointer menuMediaType">{ mediaType }</label>
    </div>
  );
