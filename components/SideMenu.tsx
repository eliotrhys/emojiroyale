...
export default function SideMenu(props: SideMenuProps) {
  ...
  const allMediaTypes = Object.values(MediaType).map((mediaType, index) => 
    <div className="mb-6" key={index}>
      ...
      <label className="inline-block pl-[0.15rem] text-sm hover:cursor-pointer">{ mediaType }</label>
    </div>
  );
    ...
    <div>
      <div className={`offcanvas-menu ${props.isOpen ? "open" : ""} px-4`}>
        ...
        <div>
          ...
          <Link href={"https://twitter.com/eliotrhys"} className="flex items-center ease-in-out transition-100 hover:scale-105">
            <div>
